...
exports.listThreads = asyncHandler(async (req, res) => {
  ...
  const threadIds = threads.map((thread) => thread._id).filter(Boolean);
  ...
  if (threadIds.length) {
    ...
    unreadCounts.forEach((row) => {
      unreadByThread.set(String(row._id), Number(row.count) || 0);
    });
  }
  ...
  const serialized = threads.map((thread) => {
    ...
    return {
      ...
      unread: unreadByThread.get(String(thread._id)) || 0,
...
exports.getMessages = asyncHandler(async (req, res) => {
  ...
  if ((thread.unreadForDoctor || 0) > 0) {
    thread.unreadForDoctor = 0;
    await thread.save();
  }
